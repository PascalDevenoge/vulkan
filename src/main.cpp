...
#pragma warning(suppress : 4458)
  ...
  void createCommandBuffers() {
    ...
    if (vkAllocateCommandBuffers(device, &allocInfo, commandBuffers.data()) !=
        VK_SUCCESS) {
      throw std::runtime_error("Could not allocate graphics command buffer");
    }
  }
  ...
  void createDescriptorSets() {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
      ...
      std::array<VkWriteDescriptorSet, 2> descriptorWrite = {descriptorWrite1,
                                                             descriptorWrite2};
  ...
  void createImage(uint32_t width, uint32_t height, uint32_t mipLevels,
    ...
    if (vkAllocateMemory(device, &allocInfo, nullptr, &imageMemory) !=
        VK_SUCCESS) {
      throw std::runtime_error("Could not allocate texture image memory");
    }
  ...
  void generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth,
    ...
    if (!(formatProperties.optimalTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT)) {
      throw std::runtime_error("Texture image format does not support linearly filtered blitting");
    }
  ...
  VkFormat findSupportedFormat(const std::vector<VkFormat> &candidates,
    ...
    throw std::runtime_error("Could not find matching supported format");
  }
  ...
  void createDepthResources() {
    ...
    createImage(
        swapChainExtent.width, swapChainExtent.width, 1, depthFormat,
  ...
  void updateUniformBuffer(uint32_t currentImage) {
    ...
    UniformBufferObject ubo{
        .model = glm::rotate(glm::mat4(1.f), time * glm::radians(30.f),
                             glm::vec3(0.f, 0.f, 1.f)),
        .view = glm::lookAt(glm::vec3(2.f, 2.f, 2.f), glm::vec3(0., 0., 0.),
                            glm::vec3(0.f, 0.f, 1.f)),
